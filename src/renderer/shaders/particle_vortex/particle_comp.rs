...
fn snoise(v: Vec2) -> f32 {
	...
	// Gradients — contribute from each simplex corner
	let m: Vec3 = vec3(
		0.5 - dot_v2(x0,         x0),
	...
	let a0: Vec3 = x - ox;
	// Normalise gradients implicitly
...
fn particle_comp(
	...
	#[push_constant(dt: f32, frame: u32, count: u32)] params: &SimParams,
	#[builtin(global_invocation_id)] gid: UVec3,
) {
	let idx = gid.x as usize;
	if idx >= params.count as usize { return; }
	// Unpack position and velocity from packed vec4
	...
	vel *= drag;
	// ── Soft boundary with inward pull ────────────────────────────────────────
	let r = pos.length();
	if r > 0.95 {
	...
	pos += vel * params.dt;
	// Repack and write out
	particles_out[idx] = vec4_2v(pos, vel);
}

pub const COMPUTE_SHADER: &str = PARTICLE_COMP_GLSL;

pub static FRAG_SPV:    &[u8] = PARTICLE_COMP_SPV;