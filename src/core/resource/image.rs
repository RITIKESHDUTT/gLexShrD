...
impl<'dev, B: Backend> Image<'dev, img_state::Undefined, B> {
	pub fn allocate_2d(
		...
		let memory = device.allocate_memory(req.size, memory_type_index)?;
		device.bind_image_memory(handle, memory, 0)?;
		Ok(Self {
			device, handle, memory, format,
			extent:Extent3D::new(extent.width(), extent.height(), 1),
			family, _state: PhantomData,
		})
	}
}
...
//   same-queue ops  → pass vk::QUEUE_FAMILY_IGNORED for both
//   cross-queue release → src = owning family, dst = target family
//   cross-queue acquire → src = prior family,  dst = acquiring family
// ─────────────────────────────────────────────────────────────
...
fn retype<'dev, Old, New, B: Backend>(img: Image<'dev, Old, B>) -> Image<'dev, New, B> {
	let img = ManuallyDrop::new(img);
	Image {
		device: img.device, handle: img.handle, memory: img.memory,
		format: img.format, extent: img.extent, family: img.family,
		_state: PhantomData,
	}
}

fn retype_swapchain<'dev, Old, New, B: Backend>(img: SwapchainImage<'dev, Old, B>) -> SwapchainImage<'dev, New, B> {
	SwapchainImage {
		device: img.device, handle: img.handle, extent: img.extent,
		_state: PhantomData,
	}
}
...
impl<'dev, B: Backend> Image<'dev, img_state::Undefined, B> {
	pub fn into_transfer_dst(mut self, cmd: &CommandBuffer<'_, Recording, B>) -> Image<'dev, img_state::TransferDst,
		B> {
	...
	pub fn into_color_attachment(mut self, cmd: &CommandBuffer<'_, Recording, B>) -> Image<'dev,
		img_state::ColorAttachment, B> {
		image_barrier(self.device, cmd, self.handle,
...
impl<'dev, B: Backend> Image<'dev, img_state::TransferDst, B> {
	...
	pub fn release_to_shader_read(self, cmd: &CommandBuffer<'_, Recording, B>, dst_family: u32) -> Image<'dev,
		img_state::ShaderReadOnly, B> {
		image_barrier(self.device, cmd, self.handle,
					  ImageLayout::TransferDst, ImageLayout::ShaderReadOnly,
					  Stage::Transfer, Access::TransferWrite,
					  Stage::Fragment, Access::SampledRead,
...
impl<'dev, B: Backend> Image<'dev, img_state::ColorAttachment, B> {
	pub fn into_present_src(mut self, cmd: &CommandBuffer<'_, Recording, B>) -> Image<'dev, img_state::PresentSrc, B>
	{
		image_barrier(self.device, cmd, self.handle,
...
impl<'dev, B: Backend> SwapchainImage<'dev, img_state::Undefined, B> {
	pub fn into_color_attachment(self, cmd: &CommandBuffer<'_, Recording, B>) -> SwapchainImage<'dev,
		img_state::ColorAttachment, B> {
		self.device.cmd_image_barrier(cmd.handle(), &[ImageBarrierInfo {
			image: self.handle,
			old_layout: ImageLayout::Undefined,
			new_layout: ImageLayout::ColorAttachment,
			src_stage: Stage::ColorOutput, src_access: Access::None,
			dst_stage: Stage::ColorOutput, dst_access: Access::ColorAttachmentWrite,
			aspect: ImageAspect::Color,
			src_queue_family: QUEUE_FAMILY_IGNORED, dst_queue_family: QUEUE_FAMILY_IGNORED,
		}]);
...
impl<'dev, B: Backend> SwapchainImage<'dev, img_state::ColorAttachment, B> {
	pub fn into_present_src(self, cmd: &CommandBuffer<'_, Recording, B>) -> SwapchainImage<'dev,
		img_state::PresentSrc, B> {
		self.device.cmd_image_barrier(cmd.handle(), &[ImageBarrierInfo {
			image: self.handle,
			old_layout: ImageLayout::ColorAttachment,
			new_layout: ImageLayout::Present,
			src_stage: Stage::ColorOutput, src_access: Access::ColorAttachmentWrite,
			dst_stage: Stage::Bottom, dst_access: Access::None,
			aspect: ImageAspect::Color,
			src_queue_family: QUEUE_FAMILY_IGNORED, dst_queue_family: QUEUE_FAMILY_IGNORED,
		}]);
		retype_swapchain(self)
